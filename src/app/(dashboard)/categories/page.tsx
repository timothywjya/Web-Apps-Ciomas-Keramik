      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '20px' }}>
          {categories.map((c, i) => (
            <div key={c.id} className="card" style={{ position: 'relative', overflow: 'hidden' }}>
              <div style={{
                ...
                fontSize: '5rem', opacity: 0.04, lineHeight: 1,
              ...
              <div style={{
                ...
                color: '#d4a843', fontSize: '1.4rem', marginBottom: '16px',
              }}>
                {categoryIcons[i % categoryIcons.length]}
              </div>
              <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.25rem', fontWeight: 600, marginBottom: '6px' }}>
                {c.name}
              </h3>
              <p style={{ fontSize: '0.82rem', color: '#78716c', marginBottom: '20px', minHeight: '36px' }}>
                {c.description || 'Tidak ada deskripsi'}
              </p>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => openEdit(c)} style={{ background: '#fef3c7', border: 'none', borderRadius: '6px', padding: '7px 14px', cursor: 'pointer', fontSize: '0.75rem', color: '#92400e', fontWeight: 500 }}>
                  Edit
                </button>
                <button onClick={() => handleDelete(c.id)} className="btn-danger" style={{ padding: '7px 14px' }}>
                  Hapus
                </button>
              </div>
            </div>
          ...
          <div onClick={openAdd} style={{
            border: '2px dashed #e7e5e4', borderRadius: '12px', padding: '24px',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
            cursor: 'pointer', transition: 'all 0.2s', minHeight: '180px',
            color: '#a8a29e',
          }}
